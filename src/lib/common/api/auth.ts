...
/**
 * Register a node for authentication (Headscale 0.29+).
 * This endpoint allows pre-registering a node before it connects.
 */
export async function authRegister(user: string, authId?: string): Promise<Node> {
    const data: ApiAuthRegisterRequest = { user };
    if (authId) {
        data.authId = authId;
    }
    const { node } = await apiPost<ApiAuthRegisterResponse>(
