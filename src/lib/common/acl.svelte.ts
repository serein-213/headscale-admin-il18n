...
export type AclGrants = Record<string, unknown>[];
export type AclNodeAttrs = Record<string, unknown>[];
export type AclTests = Record<string, unknown>[];
export type AclSshTests = Record<string, unknown>[];
...
export class ACLBuilder implements ACL {
	...
	grants = $state<AclGrants | undefined>(undefined);
	nodeAttrs = $state<AclNodeAttrs | undefined>(undefined);
	tests = $state<AclTests | undefined>(undefined);
	sshTests = $state<AclSshTests | undefined>(undefined);
	...
	JSON(space: number = 0): string {
		...
		if (this.ssh !== undefined) policy.ssh = this.ssh;
		if (this.grants !== undefined) policy.grants = this.grants;
		if (this.nodeAttrs !== undefined) policy.nodeAttrs = this.nodeAttrs;
		if (this.tests !== undefined) policy.tests = this.tests;
		if (this.sshTests !== undefined) policy.sshTests = this.sshTests;
		if (this.randomizeClientPort !== undefined) {
			policy.randomizeClientPort = this.randomizeClientPort;
		}
	...
	static fromPolicy(acl: ACL | string): ACLBuilder {
		...
		const grants = acl.grants ? [...acl.grants] : undefined;
		const nodeAttrs = acl.nodeAttrs ? [...acl.nodeAttrs] : undefined;
		const tests = acl.tests ? [...acl.tests] : undefined;
		const sshTests = acl.sshTests ? [...acl.sshTests] : undefined;
	...
	normalize() {
		...
		for (const tag in this.tagOwners) {
			this.tagOwners[tag] = this.tagOwners[tag].map((id) =>
				ACLBuilder.normalizeIdentifier(id, false),
			);
		}
	...
	private static normalizeIdentifier(id: string, isHost: boolean): string {
		if (
			...
			id.includes('@') ||
			id.includes('/') ||
			id.startsWith('autogroup:') ||
			ACLBuilder.getPrefix(id) !== null ||
			isValidIP(id) ||
			isHost
		) {
			return id;
	...
	getAdvancedPolicyField(
		field: 'grants' | 'nodeAttrs' | 'tests' | 'sshTests',
	): Record<string, unknown>[] | undefined {
		return this[field];
	}

	setAdvancedPolicyField(
		field: 'grants' | 'nodeAttrs' | 'tests' | 'sshTests',
	...
	setRandomizeClientPort(value: boolean | undefined) {
		this.randomizeClientPort = value;
	}
	...
	setSshRuleAction(idx: number, action: AclSshRule['action']) {
		this.validateSshRuleIndex(idx);
		if (this.ssh !== undefined) {
			this.ssh[idx].action = action;
		}
	}

	setSshRuleCheckPeriod(idx: number, checkPeriod: string | undefined) {
		this.validateSshRuleIndex(idx);
		if (this.ssh !== undefined) {
			if (checkPeriod === undefined || checkPeriod.trim() === '') {
				delete this.ssh[idx].checkPeriod;
			} else {
				this.ssh[idx].checkPeriod = checkPeriod;
			}
		}
	}

	setSshRuleAcceptEnv(idx: number, acceptEnv: string[] | undefined) {
		this.validateSshRuleIndex(idx);
		if (this.ssh !== undefined) {
			if (acceptEnv === undefined || acceptEnv.length === 0) {
				delete this.ssh[idx].acceptEnv;
			} else {
				this.ssh[idx].acceptEnv = acceptEnv;
			}
		}
	}
